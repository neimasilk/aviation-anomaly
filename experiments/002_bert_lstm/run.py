...
Or from project root:
    python -m experiments.002_bert_lstm.run

This experiment extends the baseline by modeling sequential dependencies:
- Groups utterances into sequences
- Uses BERT to encode each utterance
- Uses Bi-LSTM to model temporal patterns
- Uses attention to focus on important utterances
"""
import json
import sys
...
from src.models.bert_lstm import BertLSTMClassifier, BertLSTMPredictor, create_tokenizer
...
class Experiment002Runner:
    """Runner for Experiment 002."""
    ...
    def print_info(self):
        ...
        table.add_row("LSTM Hidden", str(self.config["model"]["lstm_hidden"]))
        table.add_row("LSTM Layers", str(self.config["model"]["lstm_layers"]))
        table.add_row("", "")
    ...
    def run(self):
        ...
        # Load model
        console.print(f"[cyan]Creating BERT+LSTM model...[/cyan]")
        ...
        console.print("\n[bold cyan]Comparison with Baseline (Exp 001):[/bold cyan]")
        baseline_acc = 0.6482
        baseline_f1 = 0.4734
        acc_diff = test_metrics['accuracy'] - baseline_acc
        f1_diff = test_metrics['f1_macro'] - baseline_f1

        if acc_diff > 0:
            console.print(f"  Accuracy: [green]+{acc_diff:.4f}[/green] ({test_metrics['accuracy']:.4f} vs {baseline_acc:.4f})")
        else:
            console.print(f"  Accuracy: [red]{acc_diff:.4f}[/red] ({test_metrics['accuracy']:.4f} vs {baseline_acc:.4f})")

        if f1_diff > 0:
            console.print(f"  Macro F1: [green]+{f1_diff:.4f}[/green] ({test_metrics['f1_macro']:.4f} vs {baseline_f1:.4f})")
        else:
            console.print(f"  Macro F1: [red]{f1_diff:.4f}[/red] ({test_metrics['f1_macro']:.4f} vs {baseline_f1:.4f})")
